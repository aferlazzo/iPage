...
function PaginateDisplay($Row, $Link, $UserLevel, $ReportDate, $CountOrDisplay)
{
	...
	list($month, $day, $year) = sscanf($ReportDate, "%02d/%02d/%02d");// reformat MM/DD/YY date
...
*/
	...
	while ($Row=mysql_fetch_array($Result)) //fetch each record in the table
	{
		...
		list($month, $day, $year) = sscanf($Row[ReportDate], "%02d/%02d/%02d");// reformat MM/DD/YY date
		$CompareDate = sprintf("%02d%02d%02d", $year, $month, $day);			// to YYMMDD
		...
		if (($x < $CommentLimit) || ($CommentLimit == "All")) //If the CommentLimit has NOT been exceeded
		{
			...
			if (PaginateDisplay($Row, $Link, $UserLevel, $Row[ReportDate], 'Count')) //returns 'TRUE' if record meets all the search criteria
			{
	...
	if ($TotalRows < $RowsPerPage)  //if the number of rows do not warrant at least second page...
	{
		...
		//print ("Screen $TotalPages (Single screen), has rows $StartRow - $EndRow; ");
	}
	else	//Multiple pages 
	{
	...
	while (($Row=mysql_fetch_array($Result)) && ($i <= $EndRow) && ($y < $RowsPerPage))//print up to 9 rows
	{
		...
		if ($LastApplicantNamePrinted != $Row[ApplicantName]) //each time Applicant Name changes reset the counter $CommentsOfApplicantPrinted
		{
			$CommentsOfApplicantPrinted=0;
			$LastApplicantNamePrinted = $Row[ApplicantName];
		}
		...
		if (($CommentsOfApplicantPrinted < $CommentLimit) || ($CommentLimit == "All")) //If the CommentLimit has NOT been exceeded
		{
			...
			if (PaginateDisplay($Row, $Link, $UserLevel, $Row[ReportDate], 'Display'))
			{
