...
var MonthSelector="dropdown";//Month selector can be "arrow" or "dropdown", case insensitive. [arrow|dropdown]
...
function NewCal(pCtrl,pFormat,pShowTime,pTimeMode,pScroller)
{
	...
	if (exDateTime!="")//Parse Date String
	{
		...
		//parse year
		YearPattern=/^\d{4}$/;
		//---------------------Modification done in 1.6a------------------
		if (YearPattern.test(strYear))
		{
			Cal.Year=parseInt(strYear,10);
			if (Cal.Year<YearFrom)
			{	Cal.Year=YearFrom;}			
			else if (Cal.Year>YearTo)
			{	Cal.Year=YearTo;}			
...
function RenderCal()
{
	...
	if (Cal.Scroller=="DROPDOWN")
	{
		...
		//-------------------Modification done in 1.6a------------------------
		vCalHeader+="\n<td align='right'>";
		vCalHeader+="<select name=\"YrSelector\" onChange=\"javascript:winMain.Cal.SwitchYr(this.value);winMain.RenderCal();\">\n";
		for (i=YearFrom;i<=YearTo;i++)
		{
			if (i==Cal.Year)
				...
				SelectedStr="";
			vCalHeader += "<option "+SelectedStr+" value="+i+" >"+i+"\n";		
		}
		vCalHeader+="</select></td>";
