...
var error_DOB;	//birthday

var error_county;
var error_en;	//employer name
var error_in;	//income
var error_enl;	//length of employment
var error_pos;	//position
var error_cr;	//credit rating
var error_crp;	//credit payments
var error_la;	//requested loan amount
var error_lt;	//requested loan type
var error_lr;	//requested loan request
var error_ref;	//how referred
var errormsg;
...
var isDobReq = true; //birthday

var isCountyReq = true;
var isEnReq = true; //employer name
var isInReq = true; //income
var isEnlReq = true; //length of employment
var isPosReq = true; //position
var isCrReq = true; //credit rating
var isCrpReq = true; //credit payments
var isLaReq = true; //requested loan amount
var isLtReq = true; //requested loan type
var isLrReq = true; //requested loan request
var isRefReq = true; //how referred
...
function reset_error() 
{
	...
	error_DOB = false; //birthday
	error_county = false;
	error_en = false; //employer name
	error_in = false;
	error_enl = false;
	error_pos = false;
	error_cr = false;
...
function validate_signature() 
{
	...
	if ((SpaceFound != "y" && isSignatureReq == true) || (isSignatureReq == true && d.main.Signature.value == "")) 
	{
		...
		error_sig = true;
		if ((typeof(d.main.Signature.value) == undefined) || (d.main.Signature.value == '')) d.main.Signature.value = ' ';
...
function validate_email() {
		...
		if (d.main.email.value == "") {
			...
			if (error_m) {
				if ((!error_n) && (!error_ad) && (!error_c)) {
					errormsg += 'Please enter a valid E-mail Address: (e.g. john@yahoo.com)\n';
					d.main.email.focus();
					/* select highlights the existing data, focus just activates the box */
					d.main.email.select();
				}
			}
		}
	...
	/* e-mail is not required, so just return */
	{
		return;
	}
}
...
function validate_ss() {
	...
	if (isSSReq) {
		...
		for (var i = 0; i < d.main.SSN.value.length; i++)
		/* for each character in the SSN string...*/
		{
			for (var j = 0; j < numChars.length; j++)
			/* for each 'number' character...*/
			{
				...
				if (numChars.charAt(j) == d.main.SSN.value.charAt(i))
				/* if the SSN character is a 'number' */
				{
					break;
					/* ... then break out of the for loops */
				}
				else {
...
*/
		...
		for (var i = 0; i < d.main.Co_SSN.value.length; i++)
		/* for each character in the Co_SSN string...*/
		{
			for (var j = 0; j < numChars.length; j++)
			/* for each 'number' character...*/
			{
				...
				if (numChars.charAt(j) == d.main.Co_SSN.value.charAt(i))
				/* if the Co_SSN character is a 'number' */
				{
					break;
					/* ... then break out of the for loops */
				}
...
function FormatPhone(Phone) {
	...
	if (Phone == "")
	/* we already tested before coming here that the work phone existed */
	{
...
function FormatDate(MyDate) {
	doError("Start FormatDate, MyDate = |" + MyDate + "|");
...
//alert(MyDate);
	...
	doError("NewDate=|" + NewDate + "| $1 =|" + RegExp.$1 + "| $3=|" + RegExp.$3 + "| $5=|" + RegExp.$5 + "|");
	MM = RegExp.$1;
	DD = RegExp.$3;
	YY = RegExp.$5;

	if (MM.length == 1) MyMM = "0" + MM;
	else MyMM = MM;

	if (DD.length == 1) MyDD = "0" + DD;
	else MyDD = DD;

	if (YY.length == 2) MyYY = "19" + YY;
	else MyYY = YY;

	NewDate = MyMM + '-' + MyDD + '-' + MyYY
	if (NewDate == "--")
		NewDate = "";
//alert(NewDate + "=" +  MyMM + '-' + MyDD + '-' + MyYY);
	doError("Old Date |" + OldDate + "|");
	doError("New Formatted Date |" + NewDate + "| length is " + NewDate.length);
	...
	for (var i = 0; i < d.main.HomePhone.value.length; i++) {
		for (var j = 0; j < numChars.length; j++) {
			if (numChars.charAt(j) == d.main.HomePhone.value.charAt(i)) {
				...
				if (j == (numChars.length - 1) && (d.main.HomePhone.value.charAt(i) != '-'))
				/* if not at end of table & not a dash */
				{
	...
	for (var i = 0; i < d.main.EstimatedValue.value.length; i++) {
		for (var j = 0; j < numChars.length; j++) {
			if (numChars.charAt(j) == d.main.EstimatedValue.value.charAt(i)) {
				...
				if (j == (numChars.length - 1) && !(d.main.EstimatedValue.value.charAt(i) == '$') && !(d.main.EstimatedValue.value.charAt(i) == ','))
				/* if not at end of table & not a dollar sign or comma */
				{
...
function validate_1st_balance() {
	...
	if ((x == "") || (isNaN(x))) d.main.LoanBalanceOn1st.value = 0;
	else d.main.LoanBalanceOn1st.value = parseInt(x);
	...
	for (var i = 0; i < d.main.LoanBalanceOn1st.value.length; i++) {
		for (var j = 0; j < numChars.length; j++) {
			if (numChars.charAt(j) == d.main.LoanBalanceOn1st.value.charAt(i)) {
				...
				if (j == (numChars.length - 1) && !(d.main.LoanBalanceOn1st.value.charAt(i) == '$') && !(d.main.LoanBalanceOn1st.value.charAt(i) == ','))
				/* if not at end of table & not a dollar sign or comma */
				{
...
function validate_DOB() {
	doError("Start validate_birthday, DOB = |" + d.main.DOB.value + "|");
	if (isDobReq) {
/*
		var x = d.main.DOB.value;
		x = x.replace(/\//g, ""); //remove all forward slashes 
		x = x.replace(/\-/g, ""); //remove all dashes
		if ((x == "") || (isNaN(x))) {
			d.main.DOB.value = "";
			...
			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p)) {
				d.main.DOB.focus();
				d.main.DOB.select();
			}
		}
...
*/
			for (var i = 0; i < d.main.DOB.value.length; i++) {
				for (var j = 0; j < numChars.length; j++) {
					if (numChars.charAt(j) == d.main.DOB.value.charAt(i)) {
						break;
					}
					else {
						if (j == (numChars.length - 1) && !(d.main.DOB.value.charAt(i) == '-'))
						/* if not at end of table & not a dash */
						{
							errormsg += '"' + d.main.DOB.value.charAt(i) + '"' + ' is an invalid character for Date of Birth.\n';
							error_DOB = true;
						}
					}
				}
...
/* Make sure the County field has been entered.  */
function validate_PropertyCounty() {
	doError("Start validate_PropertyCounty, PropertyCounty = |" + d.main.PropertyCounty.value + "|");

	if (isCountyReq) {
		if (d.main.PropertyCounty.value == "") {
			errormsg += 'Please enter the county.\n';
			error_county = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB)) {
				d.main.PropertyCounty.focus();
				d.main.PropertyCounty.select();
			}
		}
	}
}
...
/* Make sure the Employer name field has been entered.  */
function validate_Employer() {
	doError("Start validate_Employer, Employer = |" + d.main.Employer.value + "|");

	if (isEnReq) {
		if (d.main.Employer.value == "") {
			errormsg += 'Please enter your employers name.\n';
			error_en = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county)) {
				d.main.Employer.focus();
				d.main.Employer.select();
			}
		}
	}
}
...
/* Make sure the MonthlyIncome field has been entered.  */
function validate_MonthlyIncome() {
	doError("Start validate_MonthlyIncome, MonthlyIncome = |" + d.main.MonthlyIncome.value + "|");

	if (isInReq) {
		if (d.main.MonthlyIncome.value == "") {
			errormsg += 'Please enter your monthly income.\n';
			error_in = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county) 
			&& (!error_en)) {
				d.main.MonthlyIncome.focus();
				d.main.MonthlyIncome.select();
			}
		}
	}
}
...
/* Make sure the Employment Length field has been entered.  */
function validate_EmployerTime() {
	doError("Start validate_EmployerTime, EmployerTime = |" + d.main.EmployerTime.value + "|");

	if (isEnlReq) {
		if (d.main.EmployerTime.value == "Null") {
			errormsg += 'Please enter how long employed.\n';
			error_enl = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county) 
			&& (!error_en) && (!error_in)) {
				d.main.EmployerTime.focus();
			}
		}
	}
}
...
/* Make sure the Job title/position field has been entered.  */
function validate_Position() {
	doError("Start validate_Position, Position = |" + d.main.Position.value + "|");

	if (isPosReq) {
		if (d.main.Position.value == "") {
			errormsg += 'Please enter job title/position.\n';
			error_pos = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			 && (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county) 
			 && (!error_en) && (!error_in) && (!error_enl)) {
				d.main.Position.focus();
				d.main.Position.select();
			}
		}
	}
}
...
/* Make sure the estimated credit rating field has been entered.  */
function validate_EstCreditRating() {
	doError("Start validate_EstCreditRating, EstCreditRating = |" + d.main.EstCreditRating.value + "|");

	if (isCrReq) {
		if (d.main.EstCreditRating.value == "Null") {
			errormsg += 'Please enter your estimated credit rating.\n';
			error_cr = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county) 
			&& (!error_en) && (!error_in) && (!error_enl) && (!error_pos)) {
				d.main.EstCreditRating.focus();
			}
		}
	}
}
...
/* Make sure the minimum monthly credit debt field has been entered.  */
function validate_MonthlyCreditCardDebt() {
	doError("Start validate_MonthlyCreditCardDebt, MonthlyCreditCardDebt = |" + d.main.MonthlyCreditCardDebt.value + "|");

	if (isCrpReq) {
		if (d.main.MonthlyCreditCardDebt.value == "") {
			errormsg += 'Please enter your minimum Monthly Credit Card Payments.\n';
			error_crp = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county) 
			&& (!error_en) && (!error_in) && (!error_enl) && (!error_pos) && (!error_cr)) {
				d.main.MonthlyCreditCardDebt.focus();
				d.main.MonthlyCreditCardDebt.select();
			}
		}
	}
}
...
/* Make sure the requested LoanAmount field has been entered.  */
function validate_LoanAmount() {
	doError("Start validate_LoanAmount, LoanAmount = |" + d.main.LoanAmount.value + "|");

	if (isLaReq) {
		if (d.main.LoanAmount.value == "") {
			errormsg += 'Please enter your loan amount request.\n';
			error_la = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county) 
			&& (!error_en) && (!error_in) && (!error_enl) && (!error_pos) && (!error_cr) && (!error_crp)) {
...
/* Make sure the requested Finance type field has been entered.  */
function validate_FinanceRequest() {
	doError("Start validate_FinanceRequest, FinanceRequest = |" + d.main.FinanceRequest.value + "|");

	if (isLrReq) {
		if (d.main.FinanceRequest.value == "Null") {
			errormsg += 'Please enter the requested type of financing.\n';
			error_lr = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county) 
			&& (!error_en) && (!error_in) && (!error_enl) && (!error_pos) && (!error_cr) && (!error_crp) && (!error_la)) {
				d.main.FinanceRequest.focus();
...
/* Make sure the loan type field has been entered.  */
function validate_LoanType() {
	doError("Start validate_LoanType, LoanType = |" + d.main.LoanType.value + "|");

	if (isLtReq) {
		if (d.main.LoanType.value == "Null") {
			errormsg += 'Please enter the Loan Type.\n';
			error_lt = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county) 
			&& (!error_en) && (!error_in) && (!error_enl) && (!error_pos) && (!error_cr) && (!error_crp) && (!error_la)) {
				d.main.LoanType.focus();
...
/* Make sure the how referred field has been entered.  */
function validate_HowReferred() {
	doError("Start validate_HowReferred, HowReferred = |" + d.main.HowReferred.value + "|");

	if (isRefReq) {
		if (d.main.HowReferred.value == "Null") {
			errormsg += 'Please enter how referred.\n';
			error_ref = true;

			if ((!error_n) && (!error_ad) && (!error_m) && (!error_z) && (!error_s) && (!error_p) 
			&& (!error_pt) && (!error_v) && (!error_i) && (!error_p) && (!error_DOB) && (!error_county) 
			&& (!error_en) && (!error_in) && (!error_enl) && (!error_pos) && (!error_cr) && (!error_crp) && (!error_la) && (!error_lt)) {
				d.main.HowReferred.focus();
			}
		}
	}
}
...
function Count() {
	...
	if (counter > 1) {
		alert('Sometimes the servers are a bit slow. ' + 'One click is sufficient.\n\nI\'m sure the ' + 'server will respond in a few seconds.\n\n' + 'Thank you for your patience.');
		return false;
	}
...
*/
	...
	/* if there is a co=applicant */
	{
		...
		if (CoSpaceFound == "y") {
			validate_co_ss();
			/* if there is a valid co-appliant name, make sure there is a social security number, too */
		}
	}
	...
	if (d.main.LoanAmount.value) CheckLoanSize();
	...
	validate_Impounds();
	validate_DOB();
	validate_PropertyCounty();
	validate_Employer();
	validate_MonthlyIncome();
	validate_EmployerTime();
	validate_Position();
	validate_EstCreditRating();
	validate_MonthlyCreditCardDebt();
	validate_LoanAmount();
	validate_FinanceRequest();
	validate_LoanType();
	validate_HowReferred();
	d.main.DOB.value = FormatDate(d.main.DOB.value);
	d.main.Co_DOB.value = FormatDate(d.main.Co_DOB.value);
	
//alert("dob "+ d.main.DOB.value);
	doError("Start Middle validate\nerror_n " + error_n + "\nerror_sig " + error_sig + "\nerror_cn " + error_cn + 
	"\nerror_ad " + error_ad + "\nerror_st " + error_st + "\nerror_m " + error_m + "\nerror_z " + error_z + 
	"\nerror_c " + error_c + "\nerror_s " + error_s + "\nerror_cs " + error_cs + "\nerror_hp " + error_hp +
	"\nerror_wp " + error_wp + "\nerror_p " + error_p + "\nerror_pt " + error_pt + "\nerror_v " + error_v + 
	"\nerror_i " + error_i + "\nerror_b " + error_b + "\nerror_im " + error_im + "\nerror_DOB " + error_DOB + 
	"\nerror_county " + error_county + "\nerror_en " + error_en + "\nerror_in " + error_in + 
	"\nerror_enl " + error_enl + "\nerror_pos " + error_pos + "\nerror_cr " + error_cr + 
	"\nerror_crp " + error_crp + "\nerror_la " + error_la + "\nerror_lt " + error_lt + "\nerror_lr " + error_lr + "\nerror_ref " + error_ref);
			
	if (error_n || error_sig || error_cn || error_ad || error_st || error_m || error_z || error_c || error_s || error_cs || error_hp 
	|| error_wp || error_p || error_pt || error_v	|| error_i || error_b || error_im || error_DOB || error_county || error_en || error_in 
	|| error_enl || error_pos || error_cr || error_crp || error_la || error_lt || error_lr || error_ref)
		{
		error = true;
		doError("error = true");
		//alert( "1. "+errormsg);
	...
	/*
d.main.redirect.value="https://ecreditlending.com/V2.00/cp1Validation.aspx?";
d.main.redirect.value+="&coid=9202";
...
*/
	...
	if (error == true) {
		doError("errors found - Returning false");
		alert(errormsg);
		return (false);
	}
	else {
		/* take accumulated error messages and display them */
		doError("no errors found - Returning true");
		return (true);
	}
...
*/
	if (pattern.test(d.main.FinanceRequest.value) == true) if (x < 50000) {
...
alert(d.main.FinanceRequest.value +' contains Refinance\n');
*/
	if (pattern.test(d.main.FinanceRequest.value) == false) if (x < 50000) {
