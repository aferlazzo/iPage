...
//session_start(); //cron jobs don't like sessions
$UA = $_SERVER['HTTP_USER_AGENT'];  //the user agent is blank for cron jobs
//die("user agent|$UA|\n\n");
set_include_path(get_include_path() . PATH_SEPARATOR . "/home/prontoc2/public_html/lightning-mortgage.com/pr/");
...
{							
	...
	die(logMessage("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| No admin row to fetch"));
}
...
//logMessage("mailout&nbsp;&nbsp;(".__LINE__.") SMTPDebugging |$SMTPDebugging| SMTPTimeout |$SMTPTimeout|");
...
//die("$DeleteQuery");
$Result = mysql_query($DeleteQuery, $link) or die(logMessage("mailout&nbsp;&nbsp;(".__LINE__.") Could not delete old messagesSent rows using '$DeleteQuery' Error: |".mysql_error($link)."|"));
if($Result == false)
	logMessage ("mailout&nbsp;&nbsp;(".__LINE__.") Could not delete old messagesSent rows using '$DeleteQuery'");
...
function GetUserRow($rowusr)
{
	...
	$Update_Result = mysql_query($CampaignLockQuery) or die(logMessage("mailout&nbsp;&nbsp;(".__LINE__.") Could not update lock |$arid|"));
	$Future = substr($ComputedLockTime, 0, 2)." ".substr($ComputedLockTime, 2, 2).":".substr($ComputedLockTime, 4, 2).":".substr($ComputedLockTime, 6, 2);
	if ($debugIt > 999)							
		logMessage (sprintf("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| updated lock  |%03s| until |%s|", $arid, $Future));			
	...
	if ($debugIt > 999)
		logMessage ("mailout&nbsp;&nbsp;(".__LINE__.")  Session |". $_SESSION['LockKey']."| read |$Full_Name| &lt;$Email_Address&gt;");
}
...
function PrepareMessage()
{
	...
	if ($debugIt > 999)
		logMessage ("mailout&nbsp;&nbsp;(".__LINE__.") Preparing message to send");
...
{   
	...
	if ($debugIt > 999)
		logMessage ("mailout&nbsp;&nbsp;(".__LINE__.") Message body is constructed. Time to send.");
	...
	if ($Sent == false) 
	{
		if ($debugIt > 0)
		{
			...
			logMessage ("mailout&nbsp;&nbsp;(".__LINE__.") Not sent. email format $Format To: $Full_Name '$Email_Address' Subject: $MessageSubject");
...
function RecordTheSendEvent($arid, $Email, $MessageNumberInSequence, $link)
{
	...
	$Result = mysql_query($InsertQuery, $link) or logMessage("mailout&nbsp;&nbsp;(".__LINE__.") Could not insert messagesSent row using '$InsertQuery' Error: |".mysql_error($link)."|");
	if($Result == false)
		logMessage ("mailout&nbsp;&nbsp;(".__LINE__.") Could not insert messagesSent row using '$InsertQuery'");
}
	/*
...
function UpdateUserRowAfterSend($uid)
{
	...
	$resultNext = mysql_query($msgSQL) or die(logMessage("mailout&nbsp;&nbsp;(".__LINE__.") Error: $msgSQL"));
	...
	if($num_rows_msg>0) // if there is a 'next' message, read the delay for setting the time to send it
	{
		...
		//logMessage("mailout&nbsp;&nbsp;(".__LINE__.") Next msg will be MsgNo $rowNext->seqno, with a delay of $rowNext->delay days before it will be sent");
	...
	//logMessage("mailout&nbsp;&nbsp;(".__LINE__.") $Email_Address Next msg: $MessageNumberInSequence has a delay of $DelayDays days before it will be sent. TimeToSend will be ".date('m-d-y H:i:s', $NextMsgTimeToSend));
	if ($debugIt > 999)
		logMessage("mailout&nbsp;&nbsp;(".__LINE__.") Session |".$_SESSION['LockKey']."| will now update &lt;$Email_Address&gt; next msg |$MessageNumberInSequence| senddate |$NextMsgTimeToSend|");
	//need to get the uid 
	...
	{
		if ($debugIt > 1)
			logMessage("mailout&nbsp;&nbsp;(".__LINE__.")  Session |". $_SESSION['LockKey']."| updated user &lt;$Email_Address&gt; next msg |$MessageNumberInSequence| senddate |$NextMsgTimeToSend|. Autoresponder lock extended until |$ComputedLockTime|");
	}
	else
		die(logMessage("mailout&nbsp;&nbsp;(".__LINE__.")  Session |". $_SESSION['LockKey']."| ERROR: updating user &lt;$Email_Address&gt; failed."));
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -	/
//																					/
...
function SendQueuedMessages($aid, $EmailAddressFrom, $SMTPDebugging, $SMTPTimeout, $Remove_HTML, $Remove_Text, $debugIt, $MonthlySchedule)
{
	...
	if($debugIt > 999)
		logMessage("mailout&nbsp;&nbsp;(".__LINE__.") Query: $SQL_Statement<br />Found $UserCandidates candidates for sending");
	$NoMoreRows = false;	
...
//print("<p>$ucount - 1 ");
		...
		$SelectedRow = mysql_query($SQL_Statement)or die(logMessage("mailout&nbsp;&nbsp;(".__LINE__.") Could not |$SQL_Statement|"));
		...
		{
			$NoMoreRows = true;
			logMessage ("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| lost lock to session|$arrow[LockKey]|");
		}
		else //found a row
		{
//print("2 ");
			...
			{							
				...
				logMessage ("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| No user row to fetch");
			}
			$Email_Address = $rowusr[email];
			$uid = $rowusr[uid];
			...
			{
				logMessage ("mailout&nbsp;&nbsp;(".__LINE__.") A directory named mailout.stop exists. Script halted.");
				die;
			}
//print("3 ");
			/*
...
//logMessage ("mailout&nbsp;&nbsp;(".__LINE__.") Loop completed for the campaign |$arid|");
}//end funcation
// ------------------------------------------
...
for($count=0;$count<$num_rows_ar;$count++)
{
	...
	if (($arrow->LockExpiryTime < $Now) || ($arrow->LockKey == $_SESSION['LockKey']))
	{
		...
		$CampaignLockQuery  = "UPDATE autoresponders set LockExpiryTime = $ComputedLockTime, LockKey=" . $_SESSION['LockKey'];
		$CampaignLockQuery .= " WHERE arid=$arid limit 1";
		$Update_Result = mysql_query($CampaignLockQuery) or die(logMessage("mailout&nbsp;&nbsp;(".__LINE__.") Could not lock |$arid|"));
		if (($debugIt > 0) && ($lockcount == 0))	
		{			
			$lockcount++;			
			$Future = substr($ComputedLockTime, 0, 2)." ".substr($ComputedLockTime, 2, 2).":".substr($ComputedLockTime, 4, 2).":".substr($ComputedLockTime, 6, 2);
			logMessage ("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| locking  email campaigns until |$Future|");			
			//logMessage (sprintf("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| locking  |%03s| until |$ComputedLockTime|", $arid));			
		}
		...
		if (($debugIt > 0) && ($lockcount == 0))	
		{
			...
			logMessage ("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| email campaigns appear to be locked until |$Future|");			
			//logMessage (sprintf("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| appears locked |%03s| until |$arrow->LockExpiryTime|", $arid));			
		}
