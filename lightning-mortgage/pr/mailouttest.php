...
if ($_SERVER['SERVER_NAME'] == "www.lightning-mortgage.com")
{
	...
	LogIt("mailout test(".__LINE__.") on server lightning-mortgage.com");
	$SavePath="/home/users/web/b668/ipw.lightning-mortgage/public_html/";
}
else
...
{
	LogIt ("mailout&nbsp;&nbsp;(".__LINE__.")  No match for Server Name ".$_SESSION['SERVER_NAME']);			
	die("mailout() No match for Server Name ".$_SERVER['SERVER_NAME']);
}


LogIt("Test of error logging");
...
<HTML>
  <HEAD>
    <TITLE>
      mailout - Perfect Response
    </TITLE>
    <meta name="robots" content="NoIndex, NoFollow">
    <META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=iso-8859-1">
<script type='text/javascript'>
<!--
if(self.location!=top.location)  //if in a frame the script is not being executed by a person visition the web site (imagecron)
{
document.writeln("<link rel='stylesheet' type='text/css' href='./css/PerfectResponse.css'>\n");
document.writeln("<h2>Sending All Past Due Email...</h2>");
document.writeln("<div id='ProgressBarOutline'><div id='ProgressBar'></div></div>");
document.writeln("");
}
...
if($result == false)
{							
	//print("<p>mailout (".__LINE__.")  Session |". $_SESSION['LockKey']."| No admin row to fetch'</p>");	
	die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| No admin row to fetch"));
}
$num_rows = mysql_num_rows($result); 	
if ($num_rows == 0)
{							
	//print("<p>mailout (".__LINE__.")  Session |". $_SESSION['LockKey']."| No admin row to found'</p>");	
	die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| No admin row to found"));
}
//mysql_data_seek($result, 0);
$row = mysql_fetch_object($result);
$SMTPDebugging=$row->SMTPDebugging;
$SMTPTimeout=$row->SMTPTimeout;
//LogIt("mailout&nbsp;&nbsp;(".__LINE__.") SMTPDebugging |$SMTPDebugging| SMTPTimeout |$SMTPTimeout|");
...
//die("$DeleteQuery");
$Result = mysql_query($DeleteQuery, $link) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not delete old messagesSent rows using '$DeleteQuery' Error: |".mysql_error($link)."|"));
if($Result == false)
	LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") Could not delete old messagesSent rows using '$DeleteQuery'");
...
function SkipEmptyMsg ()
{
	...
	mysql_query($Update_Query) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not update user to next msg sequence number"));
	LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") Not Sent. $Email_Address had blank message, '$MessageNumberInSequence'. ");
}
function GetUserRow($rowusr)
{
	...
	$Update_Result = mysql_query($CampaignLockQuery) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not update lock |$arid|"));
	$Future = substr($ComputedLockTime, 0, 2)." ".substr($ComputedLockTime, 2, 2).":".substr($ComputedLockTime, 4, 2).":".substr($ComputedLockTime, 6, 2);
	if ($debugIt > 999)							
		LogIt (sprintf("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| updated lock  |%03s| until |%s|", $arid, $Future));			
	...
	if ($debugIt > 999)
		LogIt ("mailout&nbsp;&nbsp;(".__LINE__.")  Session |". $_SESSION['LockKey']."| read |$Full_Name| &lt;$Email_Address&gt;");
}
function GetMessageToSendAndSignature()
{
	...
	$result_msg = mysql_query($msgSQL) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Error: $msgSQL"));
	...
	if($num_rows_msg>0) // if there is a message, read the subject/body
	{
		...
		die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Couldn't read messages for campaign $arid. Check to see if campaign should be suspended. Preparing to send next message to $Email_Address, SQL statement: $msgSQL"));
...
function PrepareMessage()
{
	...
	if ($debugIt > 999)
		LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") Preparing message to send");
	...
	if (!isset($Removal_Link_Text))
		die (LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") no remove link found"));
...
{   
	...
	if ($debugIt > 999)
		LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") Message body is constructed. Time to send.");
	...
	if ($Sent == false) 
	{
		if ($debugIt > 0)
		{
			LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") autoresponder GLOBAL settings: \$arid $arid \$Display_Name $Display_Name \$EmailAddressFrom $EmailAddressFrom");
			LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") \$SMTPmailServer $SMTPmailServer \$SMTPmailbox $SMTPmailbox \$SMTPpassword $SMTPpassword \$SMTPport $SMTPport");
			LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") \$Wrap_On $Wrap_On \$Length_Of_Wrap $Length_Of_Wrap ");
			LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") Not sent. email format $Format To: $Full_Name '$Email_Address' Subject: $MessageSubject");
			//LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") Ending mailout.php");
		}
		return (false);
	}
	return (true);
...
//	whether a message was sent mailout.php has a housekeeping routine in it to 		//
...
function RecordTheSendEvent($arid, $Email, $MessageNumberInSequence, $link)
{
	...
	$Result = mysql_query($InsertQuery, $link) or LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not insert messagesSent row using '$InsertQuery' Error: |".mysql_error($link)."|");
	if($Result == false)
		LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") Could not insert messagesSent row using '$InsertQuery'");
}
	/*
...
function UpdateUserRowAfterSend($uid)
{
	...
	$resultNext = mysql_query($msgSQL) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Error: $msgSQL"));
	...
	if($num_rows_msg>0) // if there is a 'next' message, read the delay for setting the time to send it
	{
		...
		//LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Next msg will be MsgNo $rowNext->seqno, with a delay of $rowNext->delay days before it will be sent");
	...
	//LogIt("mailout&nbsp;&nbsp;(".__LINE__.") $Email_Address Next msg: $MessageNumberInSequence has a delay of $DelayDays days before it will be sent. TimeToSend will be ".date('m-d-y H:i:s', $NextMsgTimeToSend));
	if ($debugIt > 999)
		LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Session |".$_SESSION['LockKey']."| will now update &lt;$Email_Address&gt; next msg |$MessageNumberInSequence| senddate |$NextMsgTimeToSend|");
	//need to get the uid 
	...
	{
		if ($debugIt > 1)
			LogIt("mailout&nbsp;&nbsp;(".__LINE__.")  Session |". $_SESSION['LockKey']."| updated user &lt;$Email_Address&gt; next msg |$MessageNumberInSequence| senddate |$NextMsgTimeToSend|. Autoresponder lock extended until |$ComputedLockTime|");
	}
	else
		die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.")  Session |". $_SESSION['LockKey']."| ERROR: updating user &lt;$Email_Address&gt; failed."));
}
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -	/
//																					/
...
function SendQueuedMessages($aid, $EmailAddressFrom, $SMTPDebugging, $SMTPTimeout, $Remove_HTML, $Remove_Text, $debugIt, $MonthlySchedule)
{
	...
	$SelectedRows = mysql_query($SQL_Statement) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not obtain all the user rows for arid=".$arid));
	$UserCandidates = mysql_num_rows($SelectedRows);
	if($debugIt > 999)
		LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Query: $SQL_Statement<br />Found $UserCandidates candidates for sending");
	$NoMoreRows = false;	
...
//print("<p>$ucount - 1 ");
		...
		$SelectedRow = mysql_query($SQL_Statement)or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not |$SQL_Statement|"));
		// Verify I still have the lock, too
		$arSQL = "select * from autoresponders where arid=$arid";
		$result_ar = mysql_query($arSQL) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not |$arSQL|"));
		...
		{
			$NoMoreRows = true;
			LogIt ("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| lost lock to session|$arrow[LockKey]|");
		}
		else //found a row
		{
//print("2 ");
			...
			if ($rowusr == false)
			{							
				//print("<p>mailout (".__LINE__.")  Session |". $_SESSION['LockKey']."| No user row to fetch'</p>");	
				LogIt ("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| No user row to fetch");
			}
			$Email_Address = $rowusr[email];
			$uid = $rowusr[uid];
			...
			//die("mailout (".__LINE__.") path for mailout.stop: ".realpath('.'));
			//test to see if this script should be stopped by looking for a directory names mailout.stop
			if(file_exists("mailout.stop"))
			{
				LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") A directory named mailout.stop exists. Script halted.");
				die;
			}
//print("3 ");
			/*
...
/*
		print("<script type='text/javascript'>\nif(self.location != top.location)");  //if in a frame
		print("   ProgressBar($ucount, $UserCandidates, 500);\n</script>\n");
		flush();
		ob_flush();
*/
...
//LogIt ("mailout&nbsp;&nbsp;(".__LINE__.") Loop completed for the campaign |$arid|");
}//end funcation
// ------------------------------------------
$arSQL = "select * from autoresponders order by arid";
$result_ar = mysql_query($arSQL) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not |$arSQL|"));
$num_rows_ar = mysql_num_rows($result_ar) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not calculate number of rows"));
$lockcount=0;
for($count=0;$count<$num_rows_ar;$count++)
{
	mysql_data_seek($result_ar, $count) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not move pointer to next autoresponder row"));
	$arrow = mysql_fetch_object($result_ar) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not read next autoresponder row"));
	$arid = $arrow->arid;
	$Now = date ("dHis"); //returns current time in ddhhmmss format. 
	...
	if (($arrow->LockExpiryTime < $Now) || ($arrow->LockKey == $_SESSION['LockKey']))
	{
		...
		$CampaignLockQuery  = "UPDATE autoresponders set LockExpiryTime = $ComputedLockTime, LockKey=" . $_SESSION['LockKey'];
		$CampaignLockQuery .= " WHERE arid=$arid limit 1";
		$Update_Result = mysql_query($CampaignLockQuery) or die(LogIt("mailout&nbsp;&nbsp;(".__LINE__.") Could not lock |$arid|"));
		if (($debugIt > 0) && ($lockcount == 0))	
		{			
			$lockcount++;			
			$Future = substr($ComputedLockTime, 0, 2)." ".substr($ComputedLockTime, 2, 2).":".substr($ComputedLockTime, 4, 2).":".substr($ComputedLockTime, 6, 2);
			LogIt ("mailout test (".__LINE__.")  Session |".$_SESSION['LockKey']."| locking  email campaigns until |$Future|");			
			//LogIt (sprintf("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| locking  |%03s| until |$ComputedLockTime|", $arid));			
		}
		...
		if (($debugIt > 0) && ($lockcount == 0))	
		{
			...
			LogIt ("mailout test (".__LINE__.")  Session |".$_SESSION['LockKey']."| email campaigns appear to be locked until |$Future|");			
			//LogIt (sprintf("mailout&nbsp;&nbsp;(".__LINE__.")  Session |".$_SESSION['LockKey']."| appears locked |%03s| until |$arrow->LockExpiryTime|", $arid));			
		}
...
/*
print("<script type='text/javascript'>");
print("if(self.location!=top.location)");  //if in a frame
//print("	ProgressBar(0, 1, 500);\n");
print("document.writeln('<h2>Completed</h2>');\n</script>");
*/
    ?>
  </body>
</html>
